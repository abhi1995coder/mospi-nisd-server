...
exports.verifyOtp=async(req,res)=>{
   ...
   try{
       ...
       await user.update({isVerified:true,optCode:null,otpExpiresAt:null})
...
exports.resetPassword=async(req,res)=>{
   ...
   try{
    const user=await User.findOne({where:{email}})
    if(!user || user.otpCode!=otp || new Date()>user.expiresAt) return res.status(400).json({message:'Invalid otp or expired '})
    
    const hashed=await bcrypt.hash(new_password,10)
    await user.update({passwordHash:hash,otpCode:null,otpExpiresAt:null})
